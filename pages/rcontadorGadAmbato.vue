  ...
  methods: {
    ...
    async readConteoPasajeros() 
    {
      ...
      try {
        var body = {
          ...
          fechaF: getFecha_dd_mm_yyyy(this.fechaFinalConteoPasajerosVueltas)
        };

        var datos = await this.$axios.post(
          process.env.baseUrl + "/contadorPasajerosGadAmbato",
          body,
          {
            timeout: 600000,
          }
    ...
    createPDFGADAMBATO() {
      ...
      for (var i = 0; i < this.tableDataConteoGADAMBATO.length; i++) {
          ...
          table: {
            ...
            body: [
              ...
              [
                ...
                {
                  text: this.tableDataConteoGADAMBATO[i].fechaConteo,
                  alignment: "center",
                },
              ],
              [
        ...
        for (
          var j = 0;
          j < this.tableDataConteoGADAMBATO[i].datos.length;
          j++
        ) {
          listReporte.push({
            table: {
              ...
              body: [
                [
                  ...
                  {
                    text: this.tableDataConteoGADAMBATO[i].datos[j]
                      .DescRutaSali_m,
                    fontSize: fontSize1,
                    alignment: "center",
                  },
          ...
          var listaDetalleConteo = []

          listaDetalleConteo.push([
                  {
                    text: "Unidad (N° RTM)",
                    fontSize: fontSize1,
                    bold: true,
                  ...
                  {
                    text: "N° Pasajeros Tarifa Integral",
                    fontSize: fontSize1,
                    bold: true,
                    alignment: "center",
                  },
                  {
                    text: "N° Pasajeros Tarifa Reducida",
                    fontSize: fontSize1,
                    bold: true,
                    alignment: "center",
                  },
          ...
          for(var k = 0;k<this.tableDataConteoGADAMBATO[i].datos[j].datos.length;k++)
          {
            listaDetalleConteo.push([
                  {
                    text: this.tableDataConteoGADAMBATO[i].datos[j].datos[k].unidad,
                    fontSize: fontSize1,
                    alignment: "center",
                  },
                  {
                    text: this.tableDataConteoGADAMBATO[i].datos[j].datos[k].totalVueltas,
                    fontSize: fontSize1,
                    alignment: "center",
                  },
                  {
                    text: this.tableDataConteoGADAMBATO[i].datos[j].datos[k].tarifaIntegral,
                    fontSize: fontSize1,
                    alignment: "center",
                  },
                  {
                    text: this.tableDataConteoGADAMBATO[i].datos[j].datos[k].tarifaReducida,
                    fontSize: fontSize1,
                    alignment: "center",
                  },
                  {
                    text: this.tableDataConteoGADAMBATO[i].datos[j].datos[k].totalSubidas,
                    fontSize: fontSize1,
                    alignment: "center",
                  },
                ])
          ...
          listReporte.push({
            table: {
              headerRows: 1,
              //458
              widths: [108, 70, 110, 110, "*"],
              body: listaDetalleConteo
            },
          });
      ...
      pdfDocGenerator.getBlob((blob) => {
        ...
        /*let iframe = document.getElementById("iframeContainerGADAMBATO");
        iframe.src = this.Base64ReporteGAD;*/
      });
    },
  },
  mounted() {
