  ...
  methods: {
    ...
    async readConsolidadoSalidas() {
      ...
      try {
        ...
        var datos = await this.$axios.post(
          process.env.baseUrl + "/RConsolidadoSalidas",
          {
            token: this.token,
            unidades:
    ...
    genratePdf(datos) {
      ...
      const componenteHeader = (titulo) => {
        ...
        var unidades = {
          text: [
            { text: "UNIDADES : ", fontSize: 12, bold: true },
            {
              text:
                (this.itemUnidadProduccionRPagoVehiculorecibo.length <= 0
                  ? "TODAS LAS UNIDADES"
                  : this.itemUnidadProduccionRPagoVehiculorecibo.toString()),
              fontSize: 11,
              alignment: "left",
              bold: true,
            },
      ...
      for (var i = 0; i < datos.length; i++) {
        ...
        if (datos.length > 0) {
          acumulado = acumulado + datos[i].Despachos;
        }
            ...
            },
            {
              ...
              table: {
                ...
                body: [
                  [
                    {
                      text: "REPORTE DE CONSOLIDADO DE SALIDAS ",
                      alignment: "center",
                      fontSize: 16,
                      bold: true,
                    },
        ...
        content: [
          ...
          },
          {
            table: {
              headerRows: 0,
              widths: [150, 80],
              body: componenteResultado(acumulado),
            },
            margin: [0, 20, 0, 0],
          },
        ],
