...
export default {
  ...
  methods: {
    ...
    async readLineasRSalidasFrecuenciasControles() {
      ...
      var datos = await this.$axios.post(process.env.baseUrl + "/readRutaArmenia", {
        token: this.token,
      });
      if (datos.data.status_code == 200) {
        this.mListLineasFecuenciasControles.push(...datos.data.datos);
      } else {
    ...
    async readReporteSalidasControles() {
      ...
      try {
        var datos = await this.$axios.post(
          process.env.baseUrl + "/rSalidasRutaArmenia",
          {
            token: this.token,
            unidades:
              this.itemUnidadSalidasPanelBusqueda.length > 0
    ...
    async createPDFSalidasControles() {
      ...
      for (var j = 0; j < this.mListSalidasFrecuenciasControles.length; j++) {
        ...
        {
          /*console.log(this.mListControlesCabezera[k].idFrec)
          console.log(this.mListControlesCabezera[k].CodiCtrlSecuCtrl)
          console.log(this.mListSalidasFrecuenciasControles[j].idFrecSali_m)
          console.log(this.mListSalidasFrecuenciasControles[j].controles)*/
    ...
    getItemPdf(idFrecuencia, idControl, idFrecuenciaSM, controles) 
    {
      ...
      //console.log("TAM : "+controles.length)

      for (var i = 0; i < controles.length; i++) 
      {
        console.log(idControl +" == "+ controles[i].CodiCtrlSali_d)
